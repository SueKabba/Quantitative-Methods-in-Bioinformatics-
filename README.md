...
│   ├── BNFOgenes.fasta                 <- Raw genomic sequence inputs (20 donor profiles)
│   ├── TRE_Cancer_Status.csv           <- Patient clinical diagnosis metadata
│   ├── Tumor_Data.csv                  <- Phase I human clinical trial records (n=50)
...
- Genomic Library: Biostrings (Exact string matching and vector alignment)
...
- The Goal: Investigate if abnormal nucleotide elongation (the repetition of the 'CTG' motif) inside the open reading frame of the BNFO gene shares a significant link with clinical oncogenesis using sequence data from `BNFOgenes.fasta` and medical statuses from `TRE_Cancer_Status.csv`.
...
- The Goal: Evaluate whether a standard once daily IV bolus dose (200 ug/mL) effectively sustains a therapeutic blood concentration margin (>= 70 ug/mL) against natural bodily clearance.
...
- The Finding: While both approaches yield an identical expected average of 5 molecules, Method 1 creates extreme, volatile variance (Var = 75.0), leading to a low 25% delivery success rate. By spreading risk across independent vehicles, Method 2 slashes variance down to 3.75 more than tripling clinical reliability to a 77.5% success rate. Method 2 is heavily recommended for development.

### Phase 4: Longitudinal Efficacy & Clinical Trial Validation
- The Goal: Measure localized pre-treatment tissue response baseline cell counts against historical benchmarks, and evaluate real-world patient outcomes utilizing the records within `Tumor_Data.csv` .
